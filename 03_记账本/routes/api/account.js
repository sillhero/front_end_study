...
router.get('/', checkTokenMiddleware, async function (req, res, next) {
  //获取请求报文中的 token
  try {
    //读取数据库, 获取所有账单 根据个人的user_id来获取当前用户的所有账单
    let accounts = await AccountModel.find({user_id: req.user.user_id});
    //获取文档的总数量
    let total = await AccountModel.count({user_id: req.user.user_id});
    //响应结果
    res.json({
      msg: '获取成功', //提示语
